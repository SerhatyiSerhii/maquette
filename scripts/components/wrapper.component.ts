...
export class WrapperComp implements IComp {
    ...
    setWrapperChildren(): HTMLElement[] {
        ...
        while (totalPositions) {
            ...
            this.movieSectionService.addSection((movieOptions.position >= 10) ? movieOptions.position.toString() : `0${movieOptions.position}`, componenetInstance);

            if (movieOptions.hasOwnProperty('video')) {
                sliderFrames.push(movieOptions.video);
            }

            if (totalPositions === 8 || totalPositions === 5 || totalPositions === 2) {
